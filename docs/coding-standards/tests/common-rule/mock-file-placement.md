...
1. **新規テストでモックが必要になった場合**: `grep -r "createMock[ClassName]" tests/`で既存モックを検索し、存在すれば直接インポートして再利用する。存在しない場合のみモック対象クラスのtestsディレクトリに新規作成する
2. **既存モックを別のテストでも使いたい場合**: 新しいモックファイルや再エクスポート用ラッパーを作成せず、既存の共有モックパスを直接インポートする
