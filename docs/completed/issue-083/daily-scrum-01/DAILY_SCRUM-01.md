...
PLAN.mdの最初のタスク「プロダクションコード、テストコード、ドキュメント中のfrog-frame-frontをfrog-frame-frontに変更」に取り組みます。

具体的には、以下の作業を実施します:
1. プロダクションコード内の`frog-frame-front`を`frog-frame-front`に一括置換
2. テストコード内の`frog-frame-front`を`frog-frame-front`に一括置換
3. ドキュメント内の`frog-frame-front`を`frog-frame-front`に一括置換
...
## スクラム内残タスク
- [ ] プロダクションコード内の`frog-frame-front`を`frog-frame-front`に一括置換
- [ ] テストコード内の`frog-frame-front`を`frog-frame-front`に一括置換
- [ ] ドキュメント内の`frog-frame-front`を`frog-frame-front`に一括置換
...
1. `frog-frame-front`という文字列が、コードの動作に影響を与える箇所（例：API名、データベーステーブル名、外部サービスとの連携など）に使われていないか確認が必要ですか？それとも、単純にアプリ名として使われているだけで、すべて置換して問題ないでしょうか？
...
2. 一括置換の際、除外すべきファイルやディレクトリ（例：`.git`、`node_modules`、ビルド成果物など）はありますか？
  ...
  - `frog-frame-front/host-frontend-root/frontend-src-root`直下の設定ファイル
  - `frog-frame-front`直下の設定ファイル。特にDocker関係
...
### 実施内容
1. コンテキストメニューのID、タイトル、parentIdを`frog-frame-front`から`frog-frame-front`に変更
2. package.jsonのパッケージ名を`frog-frame-front`から`frog-frame-front`に変更
