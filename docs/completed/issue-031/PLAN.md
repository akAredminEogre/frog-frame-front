...
- [x] `NormalizedString.ts` 作成 ✓ 実装完了
- [x] `NormalizedString.test.ts` 作成・テスト実装 ✓ 実装完了

### ステップ 2: HtmlContent修正
- [x] `HtmlContent.ts` の `replaceWith` メソッド修正 ✓ 実装完了
  - 正規化文字列による複数回の置換ロジック実装済み
  - `findNormalizedMatch` および `findActualRangeInString` メソッド実装済み
- [x] 既存のユニットテスト修正・新規テスト追加 ✓ 実装完了

### ステップ 3: e2eテスト追加
- [x] `ignore-crlf-replace.spec.ts` 作成・実装 ✓ 実装完了

### ステップ 4: 統合テスト
- [x] 全てのユニットテストが通ることを確認 ✓ 完了（前回セッションで27テストが成功）
- [x] 全てのe2eテストが通ることを確認 ✓ 完了
- [x] 既存機能に影響がないことを確認 ✓ 完了

### ステップ 5: コードクリーンアップ（残作業）
- [ ] デバッグログの削除（`HtmlContent.ts`の`console.log`文を削除）

## 受け入れ条件チェック
- [x] 上記ユースケースのe2eテストが追加され、パスするように実装が変更されている ✓
- [x] そのe2eテストが通ること ✓
- [x] 既存のe2eテストが通ること ✓
- [x] 変更箇所については、`CODING_STYLE.md` の `# オブジェクト指向ルール` に従っていること ✓
