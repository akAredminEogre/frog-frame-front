...
1. **最新バージョンの確認**
   ```bash
   cd ~/akAredminEogre-project/favorite-keyword-link-frog && docker compose exec frontend npm show @playwright/test version
   ```
...
1. **Dockerコンテナの再ビルド**
   ```bash
   cd ~/akAredminEogre-project/favorite-keyword-link-frog && docker compose build --no-cache
   ```
