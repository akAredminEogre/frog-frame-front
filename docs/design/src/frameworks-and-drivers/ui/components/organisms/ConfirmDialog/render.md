...
| aria-labelledby | タイトルIDを参照 | スクリーンリーダーがタイトルを読み上げる |
| aria-describedby | メッセージIDを参照 | スクリーンリーダーが説明を読み上げる |
...
| 分類 | テストケース | 根拠 |
|------|-------------|------|
| Escape キー | onCancelが呼ばれる | キーボードでダイアログを閉じる |
| Tab キー | フォーカスがダイアログ内でループする | フォーカストラップの実装確認 |
| Shift+Tab キー | 逆方向にフォーカスがループする | 逆方向のフォーカストラップ |

**対応テスト**: `keyboard-events.test.tsx`
...
- [x] 表示/非表示の状態（2パターン）
- [x] Propsの反映（5パターン）
- [x] アクセシビリティ属性（4パターン）- ADR-007必須
- [x] ボタンクリックイベント（3パターン）
- [x] キーボード操作（3パターン）- ADR-007必須
