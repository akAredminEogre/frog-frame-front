...
| クラス | 責務 |
|--------|------|
| RewriteRule | ルールエンティティ。エクスポート対象のドメイン属性を保持(既存、変更なし) |
...
<!-- レビュー質問: 内部IDをエクスポートに含めるべきか？
     インポート時に新規ID採番する想定ならID不要。
     同一拡張内バックアップ・リストア用途ならID含有が望ましい。
     現設計はID除外(00-overview.md準拠)。 -->
...
| ロジック | 配置先 | 実装 |
|---------|--------|------|
| ルールの属性保持 | `enterprise-business-rules` | `RewriteRule`(既存) |
