...
**対応テスト**: `data-independence.spec.ts`
...
| 機能要件 | テストケース | テストファイル |
|---------|-------------|---------------|
| トグル操作でルールの有効/無効を切り替える | true→false、false→true切り替え | normal-cases.spec.ts |
| 変更をDBに保存する | リロード後もトグル状態が維持される | normal-cases.spec.ts |

### 競合状態の防止（00-overview.md参照）

| 要件 | テストケース | テストファイル |
|-----|-------------|---------------|
| トグル処理中はトグルスイッチを操作不可にする | トグル操作が正常に完了する | concurrency.spec.ts |
