...
1. **すべての対象ファイルで`new RewriteRule(...)`が`RewriteRule.fromParams()`に置換されていること**
   - 位置引数（4つ以上）の直接コンストラクタ呼び出しが排除されている
   - 名前付きプロパティにより各引数の意図が明確になっている

2. **ファクトリ実装ファイルとコンストラクタテストは対象外であること**
