...
### 4.1. ドメイン層の修正
- **`RewriteRule.ts` の修正**
  - `RewriteRule` 型に `isRegex?: boolean` プロパティを追加する。
- **`HtmlContent.ts` の修正**
  - `replaceWith` メソッドを修正する。
  - `rule.isRegex` が `true` の場合、`oldString` をエスケープせずにそのまま `new RegExp()` に渡す。これにより、ユーザーが入力した文字列が直接正規表現パターンとして利用される。
  - `rule.isRegex` が `false` の場合は、既存の文字列エスケープ処理を維持する。
- **`HtmlContent.test.ts` の修正**
  - 正規表現置換のテストケースを追加する。
    - `<h1>(.+)</h1>` を `<h2>$1</h2>` に置換するテスト
    - 後方参照 (`$1`) が正しく機能することを確認するテスト
  - 既存の文字列置換テストが影響を受けないことを確認する。

### 4.2. UI層の修正とUXの改善
- **ルール設定画面の変更**
  - 正規表現を利用するかどうかを選択するためのチェックボックスを追加する。
  - チェックボックスの状態を `RewriteRule` の `isRegex` プロパティに反映させる。
- **ヘルプテキストと注意事項の追加**
