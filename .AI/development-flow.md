...
02-sequence.puml（シーケンス図）
    ↓
03-directory-structure.md（ディレクトリ構成）  ← 必須
    ↓
...
**廃止するのはスケルトン実装フェーズのみ**。設計成果物（03-directory-structure.md・アローダイアグラム）は引き続き必須。

**旧フロー（廃止）**:
```text
02-sequence.puml → 03-directory-structure.md → アローダイアグラム → ADR準拠スケルトン実装 → ...
```

**新フロー（スケルトン実装を削除）**:
```text
02-sequence.puml → 03-directory-structure.md → アローダイアグラム → E2E 1シナリオ実装 → ...
```

**理由**: スケルトン実装は価値を生まない中間成果物であり、AI駆動開発のスピードを阻害する。
設計成果物（ディレクトリ構成・アローダイアグラム）は設計完了の証として必須。

**`03-directory-structure.md` の扱い**:
- **必須**: 全機能で作成すること
- ディレクトリ構成を設計段階で明確化し、実装のばらつきを防ぐ
- 参照: [docs-rules/design.md](../docs-rules/design.md)

**アローダイアグラムの扱い**:
- **必須**: 全機能で作成すること
- コンポーネント間の依存関係・データフローを図示する
- ASCII Art 形式で `03-directory-structure.md` の後続として作成する
