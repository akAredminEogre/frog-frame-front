...
4. [ ] テストコードを実装
   - **テストケース配列には型注釈を付与**（§2: テストデータの型注釈ルール）
   - **beforeEach先頭で`vi.clearAllMocks()`、afterEach末尾で`vi.resetAllMocks()`を呼び出す**（§5: モック初期化・リセットの明示的呼び出し）
   - **テストヘルパーのsetup()/cleanup()内にモック初期化・リセットを隠蔽しない**（§5: 禁止事項）
   - 参照: `docs/coding-standards/tests/common-rule.md`
