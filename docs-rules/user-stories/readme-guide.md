...
README.mdのタスクチェックリストに進捗があった場合、`network-diagram.puml`も同時に更新すること。

#### 更新ルール

1. タスクを `- [ ]` から `- [x]` に変更した場合、対応するネットワーク図のノードも完了状態に更新する
2. ネットワーク図が存在する場合のみ適用（任意ファイルのため）
...
### specファイル追加・変更時のREADMEチェックリスト同期

E2Eテストのspecファイルを**追加・分割・統合**した場合、README.mdの「E2Eテスト」チェックリストも同時に更新すること。

#### 更新ルール

1. 新しいspecファイルを追加した場合、対応するテストカテゴリをREADMEのE2Eチェックリストに追加する
2. specファイルを分割した場合、分割後のテストカテゴリが漏れなくチェックリストに反映されているか確認する
3. 同一コミットまたは同一PRでspecファイルとREADME.mdを更新する

#### 理由

- README.mdのチェックリストは実装済みE2Eテストの全体像を示す
- specファイルの追加・分割がチェックリストに反映されないと、実装範囲の把握が不正確になる

## 完了時の更新

ユーザーストーリー完了時は以下を更新すること:

1. **概要セクションの文言更新**: 現在形・未来形の記述を完了後の状態に合わせて過去形に更新
   - 例: 「〜に準拠していない。〜を更新する。」→「〜に準拠していなかったが、〜を更新した。」
2. **タスクチェックリストの更新**: すべてのタスクを完了状態（`[x]`）に更新
3. **受け入れ条件の更新**: すべての受け入れ条件を完了状態（`[x]`）に更新
