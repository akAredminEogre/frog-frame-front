...
> - **basic-rule.md への参照はマークダウンリンク形式 `[basic-rule.md](相対パス)` で記載すること**（プレーンテキストでのファイル名記載は不可）
...
> **参照**: [mock-file-placement.md](../../docs/coding-standards/tests/common-rule/mock-file-placement.md) の「モック作成前の確認手順」
